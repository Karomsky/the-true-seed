...
const translations = {
  en: {
    ...
    message: 'has successfully completed all theological modules of The True Seed Study Series',
    date: 'Date'
  },
  tl: {
    title: 'Katibayan ng Pagtatapos',
    subtitle: 'Ito ay nagpapatunay na si',
    message: 'ay matagumpay na nakatapos ng lahat ng mga teolohikong modyul ng The True Seed Study Series',
    date: 'Petsa'
  },
  es: {
    title: 'Certificado de Finalización',
    subtitle: 'Esto certifica que',
    message: 'ha completado exitosamente todos los módulos teológicos de la Serie de Estudio de The True Seed',
    date: 'Fecha'
  }
};
...
export const generateCertificate = async (userName: string, date: string, lang: 'en' | 'tl' | 'es') => {
  ...
  element.innerHTML = `
    <div style="border: 2px solid #c5a059; padding: 20px; width: 100%; height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center; position: relative;">
      ...
      <h1 style="font-size: 48px; margin: 0; color: #1a365d; text-transform: uppercase; letter-spacing: 4px;">${t.title}</h1>
      <div style="width: 100px; height: 3px; background: #c5a059; margin: 20px 0;"></div>
      
      <p style="font-size: 20px; font-style: italic; margin-bottom: 30px;">${t.subtitle}</p>
      ...
      <p style="font-size: 20px; margin-top: 30px; max-width: 500px; line-height: 1.5;">${t.message}</p>
      ...
      <div style="margin-top: 50px; display: flex; justify-content: space-between; width: 100%; padding: 0 40px;">
        <div style="text-align: center;">
          ...
          <p style="font-size: 14px; margin-top: 5px;">${t.date}: ${date}</p>
        </div>
